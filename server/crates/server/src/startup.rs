...
use crate::{
    DeploymentImpl, routes, runtime::relay_registration,
};
...
impl ServerHandle {
    ...
    /// Run the main server until the shutdown token is cancelled.
...
pub async fn start() -> anyhow::Result<ServerHandle> {
    start_with_bind("localhost:0").await
}

/// Like [`start`], but lets the caller specify the bind address for the main
/// server (e.g. `"0.0.0.0:8080"`).
pub async fn start_with_bind(main_addr: &str) -> anyhow::Result<ServerHandle> {
